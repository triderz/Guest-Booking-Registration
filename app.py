...
UNIT_NUMBER       = os.environ.get("UNIT_NUMBER")          # e.g. "4B"
...
def send_building_email(info, id_attachments):
    ...
    unit        = info.get("unit_number") or UNIT_NUMBER or "N/A"
    ...
    names_list = "\n".join(f"  • {name}" for name in guest_names) if guest_names else "  • (not provided)"

    body = f"""Dear Front Desk,

Please find below the guest registration information for the upcoming reservation at Unit {unit}.
...
Copies of government-issued identification for all guests 18 and over are attached.
...
def handle_webhook():
    ...
    context = {
        ...
        "unit":       prop_obj.get("name", prop_obj.get("internal_name", UNIT_NUMBER or "")),
    }
    ...
    has_unit   = bool(extracted.get("unit_number") or UNIT_NUMBER)
    ...
    """, (
        ...
        extracted.get("unit_number", UNIT_NUMBER or ""),
